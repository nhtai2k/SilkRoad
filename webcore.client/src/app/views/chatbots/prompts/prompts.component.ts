...
export class PromptsComponent implements OnInit {
  ...
  deleteById: number = 0;
  data: Pagination<PromptModel> = new Pagination<PromptModel>();
  trashData: Pagination<PromptModel> = new Pagination<PromptModel>();
  ...
  constructor(private promptService: PromptService, private toastService: ToastService) { }
  ...
  getData() {
    this.promptService.getAll(this.pageInformation.pageIndex, this.pageInformation.pageSize).subscribe({
      next: (res) => {
        this.data = res.data;
  ...
  onSubmitCreateForm() {
    if (this.createForm.valid) {
      this.promptService.create(this.createForm.value).subscribe({
        next: (res: any) => {
  ...
  updateData(id: number) {
    this.promptService.getById(id).subscribe({
      next: (res) => {
  ...
  onSubmitUpdateForm() {
    if (this.updateForm.valid) {
      this.promptService.update(this.updateForm.value).subscribe({
        next: (res: any) => {
          this.toggleLiveUpdateModel();
          this.getData();
          this.toastService.showToast(EColors.success, res.message);
  ...
  onConfirmDelete() {
    this.promptService.softDelete(this.deleteById).subscribe({
      next: (res: any) => {
        this.toggleLiveDelete();
        this.getData();
        this.toastService.showToast(EColors.success, res.message);
  ...
  //#region Trash
  getTrashData() {
    this.promptService.getAllDeleted(this.trashPageInformation.pageIndex, this.trashPageInformation.pageSize).subscribe({
      next: (res) => {
  ...
  restoreData(id: number) {
    this.promptService.restore(id).subscribe({
      next: (res: any) => {
        this.getTrashData();
        this.getData();
        this.toastService.showToast(EColors.success, res.message);
