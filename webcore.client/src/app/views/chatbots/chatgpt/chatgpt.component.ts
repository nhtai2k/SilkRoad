import { Component, signal, computed, ViewChild, ElementRef, OnInit, OnDestroy, AfterViewInit, effect } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormControl, FormGroup, FormsModule } from '@angular/forms';
import { Message } from '@models/chatbot-models/message.model';
import * as THREE from 'three';
import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js';
import { FBXLoader } from 'three/examples/jsm/loaders/FBXLoader.js';
import { OBJLoader } from 'three/examples/jsm/loaders/OBJLoader.js';
import { OptionModel } from '@models/option.model';
import { VoiceService } from '@services/chatbot-services/voice.service';
import { PromptService } from '@services/chatbot-services/prompt.service';
import { ChatGPTService } from '@services/chatbot-services/chatgpt.service';
...
export class ChatGPTComponent implements AfterViewInit, OnDestroy, OnInit {
  //#region properties
  //#region DOM Elements
  ...
  selectedPromptId = signal(-1); // Default server URL
  selectedModel = signal(''); // Default model
  isConnected = signal(true);
  isLoading = signal(false);
  isSpeech = signal(false);
  ...
  requestForm: FormGroup = new FormGroup({
    conversationId: new FormControl(null),
    promptId: new FormControl(-1),
  ...
  ngOnInit(): void {
    // Fetch model options
    this.chatGPTService.getModels().subscribe({
      next: (result) => {
        if (result.success) {
    ...
    this.promptService.getOptionList().subscribe({
      next: (response) => {
        if (response.success) {
          this.options = response.data;
          if (this.options.length > 0) {
            this.selectedPromptId.set(this.options[0].id);
          }
        }
  ...
  clearChat() {
    this.messages.set([]);
    this.requestForm.patchValue({ conversationId: null });
  }
  ...
  sendMessage() {
    ...
    this.chatGPTService.completeChat(this.requestForm.value).subscribe({
      next: (result) => {
        ...
        if (result.success) {
          ...
          if (this.isSpeech()) {
            this.loadVoice(result.data.response);
          }
        }
  ...
  private loadVoice(mess: string): void {
    this.voiceService.getVoiceFromChatGPT(mess).subscribe({
      next: (data) => {
