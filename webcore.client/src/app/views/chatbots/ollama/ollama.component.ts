import { Component, signal, computed, ViewChild, ElementRef, OnDestroy, AfterViewInit, OnInit, effect } from '@angular/core';
...
import { OllamaService } from '@services/chatbot-services/ollama.service';
...
export class OllamaComponent implements AfterViewInit, OnDestroy, OnInit {
  //#region properties
  //#region DOM Elements
  ...
  sendMessage() {
    ...
    this.ollamaService.completeChat(this.requestForm.value).subscribe({
      next: (result) => {
  ...
  private loadVoice(mess: string): void {
    this.voiceService.getVoiceFromElevenlabs(mess).subscribe({
      next: (data) => {
