...
export class MaterialCategoryComponent implements OnInit {
  ...
  deleteById: number = 0;
  data: Pagination<MaterialCategoryModel> = new Pagination<MaterialCategoryModel>();
  trashData: Pagination<MaterialCategoryModel> = new Pagination<MaterialCategoryModel>();
  ...
  //#region Main Table
  getData() {
    this.materialCategoryService.getAll(this.pageInformation.pageIndex, this.pageInformation.pageSize).subscribe((res) => {
      this.data = res.data;
  ...
  //#region Trash
  getTrashData() {
    this.materialCategoryService.getAllDeleted(this.trashPageInformation.pageIndex, this.trashPageInformation.pageSize).subscribe((res) => {
  ...
  restoreData(id: number) {
    this.materialCategoryService.restore(id).subscribe((res) => {
      this.getTrashData();
      this.getData();
  ...
  deleteDataTrash(id: number) {
    this.materialCategoryService.delete(id).subscribe((res) => {
      this.getTrashData();
      this.toastService.showToast(EColors.success, res.message);
    });
  }
  ...
  onSubmitCreateForm() {
    if (this.createForm.valid) {
      ...
      this.materialCategoryService.create(formData).subscribe((res) => {
  ...
  updateData(id: number) {
    this.materialCategoryService.getById(id).subscribe((res) => {
  ...
  onSubmitUpdateForm() {
    if (this.updateForm.valid) {
      ...
      this.materialCategoryService.update(formData).subscribe((res) => {
        this.toggleLiveUpdateModel();
        this.getData();
        this.toastService.showToast(EColors.success, res.message);
        this.reviewUpdateUploadImage = '';
  ...
  onConfirmDelete() {
    this.materialCategoryService.softDelete(this.deleteById).subscribe((res) => {
      this.toggleLiveDelete();
      this.getData();
      this.toastService.showToast(EColors.success, res.message);
    }, (failure) => {
