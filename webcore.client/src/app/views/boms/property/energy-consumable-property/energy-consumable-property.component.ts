...
@Component({
  selector: 'app-energy-consumable-property',
  templateUrl: './energy-consumable-property.component.html',
  styleUrl: './energy-consumable-property.component.scss',
...
export class EnergyConsumablePropertyComponent {
  ...
  createForm: FormGroup = new FormGroup({
    ...
    energyConsumptionUnitId: new FormControl(-1, Validators.required),
    energyId: new FormControl(-1, Validators.required),
    energyConsumption: new FormControl(0, [Validators.required, Validators.min(0)]),
...
onChangePrice(event: any, type: string) {
  ...
  onChangeEnergyConsumption(event: any, type: string) {
    const value = event;
    if (type === 'create') {
      this.createForm.patchValue({ energyConsumption: value });
    } else if (type === 'update') {
      this.updateForm.patchValue({ energyConsumption: value });
    }
  }
  ...
  onChangeEnergyConsumptionUnit(event: any, type: string) {
    const value = event;
    if (type === 'create') {
      this.createForm.patchValue({ energyConsumptionUnitId: value });
    } else if (type === 'update') {
      this.updateForm.patchValue({ energyConsumptionUnitId: value });
    }
  }
  //#region Create
  ...
  onSubmitCreateForm() {
    if (this.createForm.valid) {
      const formData = this.appendFormData(this.createForm, [
        'propertyGroupId', 'energyConsumptionUnitId', 'energyId', 'energyConsumption', 'unitId', 
