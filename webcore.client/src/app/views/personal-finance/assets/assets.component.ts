...
import { AssetModel, AssetTypeModel, ExpenseModel, ResourceModel } from '@models/personal-finance-models';
...
import { AssetService } from '@services/personal-finance-services/asset.service';
...
@Component({
  ...
  imports: [ModalBodyComponent, FormControlDirective, FormLabelDirective, IconDirective, ModalComponent, ButtonDirective, FormDirective, ReactiveFormsModule, ModalFooterComponent, CommonModule,
    ModalHeaderComponent, DataTableComponent, InputCurrencyComponent, SelectSearchComponent, EyeClosedIconComponent, EyeIconComponent, FormCheckComponent, InputNumberComponent]
})
export class AssetsComponent implements OnInit {
  ...
  deleteById: number = 0;
  data: Pagination<AssetModel> = new Pagination<AssetModel>();
  initAmountCreateForm = signal<number>(0);
  initAmountUpdateForm = signal<number>(0);
  initLoanAmountUpdateForm = signal<number>(0);
  ...
  ngOnInit(): void {
    ...
    this.assetTypeService.getOptionList().subscribe((res) => {
  ...
  getData() {
    this.assetService.getAll(this.pageInformation.pageIndex, this.pageInformation.pageSize, this.userId).subscribe((res) => {
      this.data = res.data;
  ...
  onSubmitCreateForm() {
    if (this.createForm.valid) {
      this.assetService.create(this.createForm.value).subscribe({
        next: (res) => {
  ...
  updateData(id: number) {
    this.assetService.getById(id).subscribe((res) => {
      ...
      this.initLoanAmountUpdateForm.set(data.loanAmount || 0);
      this.toggleLiveUpdateModel();
    });
  }
  onSubmitUpdateForm() {
    if (this.updateForm.valid) {
      this.assetService.update(this.updateForm.value).subscribe((res) => {
        this.toggleLiveUpdateModel();
        this.getData();
        this.toastService.showToast(EColors.success, res.message);
      }, (failure) => {
  ...
  onConfirmDelete() {
    this.assetService.delete(this.deleteById).subscribe((res) => {
      this.toggleLiveDelete();
      this.getData();
      this.toastService.showToast(EColors.success, res.message);
    }, (failure) => {
