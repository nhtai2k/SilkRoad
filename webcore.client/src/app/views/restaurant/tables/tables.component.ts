...
export class TablesComponent implements OnInit {
  ...
  deleteById: number = 0;
  data: Pagination<TableModel> = new Pagination<TableModel>();
  trashData: Pagination<TableModel> = new Pagination<TableModel>();
  ...
  getData() {
    this.tableService.getAll(this.pageInformation.pageIndex, this.pageInformation.pageSize).subscribe({
      next: (res) => {
        this.data = res.data;
  ...
  onSubmitCreateForm() {
    if (this.createForm.valid) {
      this.tableService.create(this.createForm.value).subscribe({
        next: (res: any) => {
  ...
  get capacityCreateForm() { return this.createForm.get('capacity'); }
  ...
  onSubmitUpdateForm() {
    if (this.updateForm.valid) {
      this.tableService.update(this.updateForm.value).subscribe({
        next: (res: any) => {
          this.toggleLiveUpdateModel();
          this.getData();
          this.toastService.showToast(EColors.success, res.message);
  ...
  get capacityUpdateForm() { return this.updateForm.get('capacity'); }
  ...
  onConfirmDelete() {
    this.tableService.softDelete(this.deleteById).subscribe({
      next: (res: any) => {
        this.toggleLiveDelete();
        this.getData();
        this.toastService.showToast(EColors.success, res.message);
  ...
  //#region Trash
  getTrashData() {
    this.tableService.getAllDeleted(this.trashPageInformation.pageIndex, this.trashPageInformation.pageSize).subscribe({
      next: (res) => {
  ...
  restoreData(id: number) {
    this.tableService.restore(id).subscribe({
      next: (res: any) => {
        this.getTrashData();
        this.getData();
        this.toastService.showToast(EColors.success, res.message);
