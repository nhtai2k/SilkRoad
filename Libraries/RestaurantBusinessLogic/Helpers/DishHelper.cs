...
namespace RestaurantBusinessLogic.Helpers
{
    public class DishHelper : IDishHelper
    {
        ...
        {
            var allItems = _unitOfWork.DishRepository.Query(x => !x.IsDeleted, orderBy: x => x.OrderBy(y => y.Priority));
            if (model.CategoryId > 0)
            {
                allItems = allItems.Where(x => x.CategoryId == model.CategoryId);
            }
            if (model.TypeId > -1)
            {
                allItems = allItems.Where(x => x.TypeId == model.TypeId);
            }
            if (!string.IsNullOrEmpty(model.Name))
            {
                allItems = allItems.Where(x => x.NameVN.Contains(model.Name));
                allItems = allItems.Where(x => x.NameEN.Contains(model.Name));
                allItems = allItems.Where(x => x.NameCN.Contains(model.Name));
            }
        ...
        public async Task<bool> CreateAsync(DishDTO model, string? username = null)
        {
            try
            {
                ...
                model.DescriptionCN = model.DescriptionCN?.Trim();
                ...
                if (model.ImageFile != null)
                {
                    if (!_fileStorageService.IsImageFile(model.ImageFile))
                        return false;
                    model.ImageUrl = _fileStorageService.SaveImageFile([EFolderNames.Dishes.ToString()], model.ImageFile);
                }
        ...
        public async Task<bool> UpdateAsync(DishDTO model, string? username = null)
        {
            try
            {
                ...
                data.DescriptionCN = model.DescriptionCN?.Trim();
                ...
                if (model.ImageFile != null)
                {
                    ...
                    {
                        _fileStorageService.DeleteFile(data.ImageUrl);
                    }
                    data.ImageUrl = _fileStorageService.SaveImageFile([EFolderNames.Dishes.ToString()], model.ImageFile);
                }
                await _unitOfWork.SaveChangesAsync();
                return true;
            }
        ...
        //        string path = Path.Combine(_webHostEnvironment.WebRootPath, EFolderNames.Templates.ToString(), "Dish.xlsx");
