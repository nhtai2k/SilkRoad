...
namespace WebCore.Server.Controllers.BOMControllers
{
    ...
    {
        private readonly IMaterialHelper _helper;
        private readonly IStringLocalizer<SharedResource> _localizer;
        public MaterialController(IMaterialHelper helper, IStringLocalizer<SharedResource> localizer) { _helper = helper; _localizer = localizer; }
        ...
        public async Task<IActionResult> GetByFilter([FromBody] MaterialFilterModel model)
        {
        ...
        [HttpGet("getByMaterialGroupId/{materialGroupId}")]
        public async Task<IActionResult> GetByMaterialGroupId(int materialGroupId)
        {
            var data = await _helper.GetByMaterialGroupIdAsync(materialGroupId);
            if (data == null)
        ...
        [HttpPost("create")]
        public async Task<IActionResult> Create([FromForm] MaterialDTO model)
        {
        ...
        [HttpPut("update")]
        public async Task<IActionResult> Update([FromForm] MaterialDTO model)
        {
            if (model == null || !ModelState.IsValid)
                return Failed(Common.EStatusCodes.BadRequest, _localizer["invalidData"]);
            var userName = User.Identity?.Name;
