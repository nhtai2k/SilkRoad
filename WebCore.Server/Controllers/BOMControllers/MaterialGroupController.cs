...
namespace WebCore.Server.Controllers.BOMControllers
{
    ...
    [Authorize]
    public class MaterialGroupController : BaseApiController
    {
        private readonly IMaterialGroupHelper _helper;
        private readonly IStringLocalizer<SharedResource> _localizer;
        public MaterialGroupController(IMaterialGroupHelper helper, IStringLocalizer<SharedResource> localizer) { _helper = helper; _localizer = localizer; }
        ...
        [HttpPost("create")]
        public async Task<IActionResult> Create([FromForm] MaterialGroupDTO model)
        {
        ...
        [HttpPut("update")]
        public async Task<IActionResult> Update([FromForm] MaterialGroupDTO model)
        {
            if (model == null || !ModelState.IsValid)
                return Failed(Common.EStatusCodes.BadRequest, _localizer["invalidData"]);
            var userName = User.Identity?.Name;
