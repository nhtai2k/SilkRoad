...
namespace WebCore.Server.Controllers.BOMControllers
{
    ...
    {
        private readonly IRankHelper _helper;
        private readonly IStringLocalizer<SharedResource> _localizer;
        public RankController(IRankHelper helper, IStringLocalizer<SharedResource> localizer) { _helper = helper; _localizer = localizer; }
        ...
        [HttpPost("Create")]
        public async Task<IActionResult> Create([FromBody] RankDTO model)
        {
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] RankDTO model)
        {
            if (model == null || !ModelState.IsValid)
                return Failed(Common.EStatusCodes.BadRequest, _localizer["invalidData"]);
            var userName = User.Identity?.Name;
