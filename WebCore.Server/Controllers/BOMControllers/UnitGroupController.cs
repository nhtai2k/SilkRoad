...
namespace WebCore.Server.Controllers.BOMControllers
{
    ...
    [Authorize]
    public class UnitGroupController : BaseApiController
    {
        private readonly IUnitGroupHelper _helper;
        private readonly IStringLocalizer<SharedResource> _localizer;
        public UnitGroupController(IUnitGroupHelper helper, IStringLocalizer<SharedResource> localizer) { _helper = helper; _localizer = localizer; }
        ...
        [HttpPost("Create")]
        public async Task<IActionResult> Create([FromBody] UnitGroupDTO model)
        {
            if (model == null || !ModelState.IsValid)
                return Failed(Common.EStatusCodes.BadRequest, _localizer["invalidData"]);
            bool isNameExists = await _helper.IsNameExistsAsync(model.Name);
            if (isNameExists)
                return Failed(Common.EStatusCodes.Conflict, _localizer["nameAlreadyExists"]);
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] UnitGroupDTO model)
        {
            if (model == null || !ModelState.IsValid)
                return Failed(Common.EStatusCodes.BadRequest, _localizer["invalidData"]);
            var userName = User.Identity?.Name;
