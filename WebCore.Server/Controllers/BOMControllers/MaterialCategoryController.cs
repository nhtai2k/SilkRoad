...
namespace WebCore.Server.Controllers.BOMControllers
{
    ...
    {
        private readonly IMaterialCategoryHelper _helper;
        private readonly IStringLocalizer<SharedResource> _localizer;
        public MaterialCategoryController(IMaterialCategoryHelper helper, IStringLocalizer<SharedResource> localizer) { _helper = helper; _localizer = localizer; }
        ...
        [HttpPost("create")]
        public async Task<IActionResult> Create([FromForm] MaterialCategoryDTO model)
        {
        ...
        [HttpPut("update")]
        public async Task<IActionResult> Update([FromForm] MaterialCategoryDTO model)
        {
            if (model == null || !ModelState.IsValid)
                return Failed(EStatusCodes.BadRequest, _localizer["invalidData"]);
            var userName = User.Identity?.Name;
