...
namespace WebCore.Server.Controllers.SurveyControllers
{
    ...
    [ApiController]
    [Authorize]
    public class PredefinedAnswerLibraryController : BaseApiController
    {
        private readonly IPredefinedAnswerLibraryHelper _helper;
        private readonly IActionLoggingService _actionLog;
        private readonly IStringLocalizer<SharedResource> _localizer;
        public PredefinedAnswerLibraryController(IPredefinedAnswerLibraryHelper helper,
        ...
        [HttpGet("getByQuestionLibraryId/{questionLibraryId}")]
        public async Task<IActionResult> getByQuestionLibraryId(int questionLibraryId)
        {
            if (questionLibraryId < 1)
            {
                return Failed(EStatusCodes.BadRequest, _localizer["invalidPageIndex"]);
            }
            var data = await _helper.GetByQuestionLibraryIdAsync(questionLibraryId);
        ...
        [HttpPost("create")]
        public async Task<IActionResult> Create([FromBody] PredefinedAnswerLibraryDTO model)
        {
        ...
        [HttpPut("update")]
        public async Task<IActionResult> Update([FromBody] PredefinedAnswerLibraryDTO model)
        {
            if (model == null || !ModelState.IsValid)
                return Failed(EStatusCodes.BadRequest, _localizer["invalidData"]);
